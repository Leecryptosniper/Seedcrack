...
def generate_seed_phrase(word_list, num_words=10):
    # Each BIP-39 word represents 11 bits of entropy
    entropy_bits = num_words * 11

    # Generate random bytes for the required entropy
    entropy_bytes = secrets.token_bytes(entropy_bits // 8)

    # Calculate the checksum
    checksum = secrets.token_bytes(entropy_bits // 8 // 4)

    # Combine entropy and checksum
    combined_data = entropy_bytes + checksum

    # Convert the combined data to a binary string
    binary_string = ''.join(format(byte, '08b') for byte in combined_data)

    # Break the binary string into 11-bit chunks
    chunks = [binary_string[i:i + 11] for i in range(0, len(binary_string), 11)]

    # Map each 11-bit chunk to a corresponding word from the word list
    words = [word_list[int(chunk, 2)] for chunk in chunks]
...
def save_seed_phrases(word_list, num_phrases, file_path):
...
save_seed_phrases(example_word_list, num_phrases_to_generate, file_path)